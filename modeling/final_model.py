...
# Function to train and tune XGBoost
...
# Function to test the model on each season
...
mens_feature_pairs = [
    ...
    ('Opp_Threes_Per_Game_1', 'Opp_Threes_Per_Game_2'),
...
def main():
    ...
    if len(sys.argv) < 4:
        print("Usage: python your_script_name.py <mens/womens> <num_features> <num_iterations>")
        sys.exit(1)

    # Parse arguments
    data_arg = sys.argv[1]  # 'mens' or 'womens'
    num_features = int(sys.argv[2])  # Number of feature pairs
    num_iterations = int(sys.argv[3])  # Number of iterations

    # Validate data argument
    if data_arg[0].lower() == 'm':  # Check first character for 'mens'
        is_mens = True
    elif data_arg[0].lower() == 'w':  # Check first character for 'womens'
        is_mens = False
    else:
        print("Invalid dataset. Specify 'mens' or 'womens'.")
        sys.exit(1)

    # Load dataset
    if is_mens:
        df = pd.read_csv("../data/modeling/mens_final_ml.csv")
        feature_pairs = mens_feature_pairs
    else:
        df = pd.read_csv("../data/modeling/womens_final_ml.csv")
        feature_pairs = womens_feature_pairs

    # Main loop to test different feature combinations
    results = []

    for i in range(num_iterations):
        selected_pairs = select_feature_pairs(feature_pairs, num_features)
        selected_features = [feature for pair in selected_pairs for feature in pair]
        print(f"Iteration {i+1}")

        #df_subset = df[['Season', 'Team1_Wins', 'reg_season_pred', 'Seed_1', 'Seed_2', 
        #            'Pace_1', 'Pace_2', 'avg_height_1', 'avg_height_2', 'offensiveRating_1', 'offensiveRating_2', 'defensiveRating_1', 'defensiveRating_2', 
        #            'Offensive_Rebound_Rate_1', 'Offensive_Rebound_Rate_2', '3PAr_1', '3PAr_2']]
    ...
    if is_mens:
        ...
        model_filename = "womens_best_xgb_model2.model"
    best_result['model'].save_model(model_filename)
    print(f"Model saved as {model_filename}")

    # Save best features and results
    if is_mens:
        best_features_filename = "mens_best_features.json"
    else:
        best_features_filename = "womens_best_features2.json"
